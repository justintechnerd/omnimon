...
namespace OmnimonModuleEditor.Tabs
{
    ...
    public partial class ModuleTab : UserControl
    {
        ...
        private class MainPanel : Panel
        {
            ...
            public TextBox txtName, txtNameFormat;
        ...
        private class UnlocksPanel : Panel
        {
            ...
            {
                return unlocks != null
                    ? new System.Collections.Generic.List<OmnimonModuleEditor.Models.Unlock>(unlocks)
                    : new System.Collections.Generic.List<OmnimonModuleEditor.Models.Unlock>();
            }
            ...
            // Manipule a conversão entre string e List<string> para a coluna "To":
            public override void Refresh()
            {
                base.Refresh();
                // Atualiza a exibição da coluna "To" para mostrar como string separada por vírgula
                foreach (DataGridViewRow row in dgvUnlocks.Rows)
                {
                    if (row.DataBoundItem is OmnimonModuleEditor.Models.Unlock unlock)
                    {
                        row.Cells["To"].Value = unlock.To != null ? string.Join(",", unlock.To) : "";
                    }
                }
            }
        }
