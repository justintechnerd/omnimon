...
from core.combat.combat_constants import (
    AFTER_ATTACK_DELAY_FRAMES, ALERT_DURATION_FRAMES, ATTACK_SPEED, BAR_HOLD_TIME_MS, ENEMY_ENTRY_SPEED,
...
class BattleEncounter:
    ...
    def update_entry(self):
        ...
        all_in_position = self.battle_player.update_entry()
    ...
    def process_battle_results(self):
        ...
        for i, pet in enumerate(self.battle_player.team1):
            ...
            pet.finish_battle(self.victory_status == "Victory", self.battle_player.team2[0], self.area)
    ...
    def setup_pet_attack(self, pet):
        ...
        if defender_idx < len(self.battle_player.team2):
            ...
            target_x = target_enemy.x + ((PET_WIDTH * BOSS_MULTIPLIER if self.boss else PET_WIDTH) // 2)
        ...
        if self.module.ruleset in ["dmc", "penc"] and self.module.battle_damage_limit < 3:
            ...
            if hits == 4:
                ...
                if hits > 1:
                    self.battle_player.team1_projectiles[pet_index].append([rotated_sprite.copy(), [x - (20 * UI_SCALE), y - (10 * UI_SCALE)], [target_x - (20 * UI_SCALE), target_y - (10 * UI_SCALE)], attack_entry])
                if hits == 3:
                    self.battle_player.team1_projectiles[pet_index].append([rotated_sprite.copy(), [x - (40 * UI_SCALE), y + (10 * UI_SCALE)], [target_x - (40 * UI_SCALE), target_y + (10 * UI_SCALE)], attack_entry])
    ...
    def setup_enemy_attack(self, enemy):
        ...
        x = self.battle_player.team2[enemy_index].x + (PET_WIDTH * BOSS_MULTIPLIER if self.boss else PET_WIDTH) // 2 - (2 * UI_SCALE)
        ...
        for attack_entry in attack_entries:
            ...
            if defender_idx < len(self.battle_player.team1):
                target_pet_x = SCREEN_WIDTH - PET_WIDTH // 2 - (2 * UI_SCALE)
            ...
            if self.module.battle_damage_limit < 3:
                ...
                if hits == 4:
                    ...
                    self.battle_player.team2_projectiles[defender_idx].append([rotated_sprite, [x, y], [target_pet_x, target_pet_y], attack_entry])
                    if hits > 1:
                        self.battle_player.team2_projectiles[defender_idx].append([rotated_sprite, [x + (20 * UI_SCALE), y + (10 * UI_SCALE)], [target_pet_x + (20 * UI_SCALE), target_pet_y + (10 * UI_SCALE)], attack_entry])
                    if hits == 3:
                        self.battle_player.team2_projectiles[defender_idx].append([rotated_sprite, [x + (40 * UI_SCALE), y - (10 * UI_SCALE)], [target_pet_x + (40 * UI_SCALE), target_pet_y - (10 * UI_SCALE)], attack_entry])
    ...
    def update_battle_pet_projectiles(self):
        ...
        for i, main_data in enumerate(self.battle_player.team1_projectiles):
            ...
            for sprite_data in main_data:
                ...
                new_pos = self.move_towards(pos, target, ATTACK_SPEED * (30 / FRAME_RATE))
            ...
            if done:
                ...
                if hit:
                    ...
                    enemy_x = enemy.x + (PET_WIDTH * BOSS_MULTIPLIER if self.boss else PET_WIDTH) // 2
                    self.hit_animations.append([0, [enemy_x, enemy_y + (16 * UI_SCALE)]])
        ...
        for i, main_data in enumerate(self.battle_player.team2_projectiles):
            ...
            if done:
                ...
                if hit:
                    ...
                    pet_x = SCREEN_WIDTH - PET_WIDTH // 2 - (2 * UI_SCALE)
                    ...
                    if defender_idx >= 0 and defender_idx < len(self.battle_player.team1):
                        pet_y = self.get_y(defender_idx, len(self.battle_player.team1))
                        pet_x = SCREEN_WIDTH - PET_WIDTH - (2 * UI_SCALE)
    ...
    def update_result(self):
        ...
        if self.result_timer < int(120 * (FRAME_RATE / 30)):
            # pisca aviso clear
            return
    ...
    def draw_clear(self, surface):
        ...
        """
        if self.battle_player.victory_status == "Victory" and self.boss:
    ...
    def draw_result(self, surface):
        ...
        spacing = min((SCREEN_WIDTH - int(30 * UI_SCALE)) // total, sprite_width + int(16 * UI_SCALE))
        total_width = spacing * total
        offset_x = (SCREEN_WIDTH - total_width) // 2
        y = int(86 * UI_SCALE)
        for i, pet in enumerate(pets):
            anim_toggle = (self.battle_player.frame_counters[i] + i * 5) // (15 * FRAME_RATE / 30) % 2
        ...
        for i, pet in enumerate(pets):
            ...
            if not self.battle_player.level_up[i] and self.battle_player.team1[i].level == MAX_LEVEL[self.battle_player.team1[i].stage]:
                exp = 0
    ...
    def draw_enemies(self, surface: pygame.Surface):
        ...
        for i, enemy in enumerate(self.battle_player.team2):
            ...
            x = enemy.x
            if attack_entry and self.battle_player.phase[i] == "enemy_charge":
    ...
    def draw_pets(self, surface: pygame.Surface):
        ...
        if self.phase == "result":
            # Horizontal layout
            spacing = min((SCREEN_WIDTH - int(30 * UI_SCALE)) // total, int(PET_WIDTH * UI_SCALE) + int(16 * UI_SCALE))
            total_width = spacing * total
            offset_x = (SCREEN_WIDTH - total_width) // 2
            y = (SCREEN_HEIGHT - PET_HEIGHT) // 2
            for i, pet in enumerate(self.battle_player.team1):
                anim_toggle = (self.battle_player.frame_counters[i] + i * 5) // (15 * FRAME_RATE / 30) % 2
                if self.victory_status == "Defeat":
                    frame_id = PetFrame.LOSE.value
    ...
    def draw_health_bars_for_battlers(self, surface):
        ...
        bar_offset_y = int(PET_HEIGHT * UI_SCALE) - int(6 * UI_SCALE)
        green = (0, 255, 108)
        red = (181, 41, 41)
        x_color = (255, 0, 0)
        x_thickness = max(2, int(2 * UI_SCALE))
        ...
        for i, pet in enumerate(self.battle_player.team1):
            ...
            if current_hp > 0:
                if self.battle_player.team1_bar_counters[i] > 0:
                    ...
                    pet_bar_width = int(PET_WIDTH * pet_hp_ratio)
                    pygame.draw.rect(surface, red, (pet_x, pet_y, PET_WIDTH, bar_height))
                ...
                start1 = (pet_x, pet_y)
                end1 = (pet_x + PET_WIDTH, pet_y + PET_HEIGHT)
                start2 = (pet_x + PET_WIDTH, pet_y)
                end2 = (pet_x, pet_y + PET_HEIGHT)
        ...
        if self.boss:
            bar_offset_y = int(PET_HEIGHT * BOSS_MULTIPLIER * UI_SCALE) - int(6 * UI_SCALE)
        for i, enemy in enumerate(self.battle_player.team2):
            enemy_x = enemy.x
            ...
            width = PET_WIDTH * BOSS_MULTIPLIER if self.boss else PET_WIDTH
            heigh = PET_HEIGHT * BOSS_MULTIPLIER if self.boss else PET_HEIGHT
            if current_hp > 0:
    ...
    def simulate_global_combat(self):
        ...
        for i, enemy in enumerate(self.battle_player.team2):
            ...
            enemy_level = MAX_LEVEL.get(enemy_stage_index, 1)
