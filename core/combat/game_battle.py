import random
from core.combat.combat_constants import ENEMY_ENTRY_SPEED
from core.constants import FRAME_RATE, SCREEN_WIDTH
...
class GameBattle:
    ...
    def update_entry(self):
        ...
        for pet in self.team2:
            if pet.x < target_x:
                pet.x += ENEMY_ENTRY_SPEED * (30 / FRAME_RATE)  # Frame-rate independent speed
    ...
    def reset_cooldowns(self):
        ...
        """
        self.cooldowns = [random.randint(40 * (FRAME_RATE / 30), 60 * (FRAME_RATE / 30)) for _ in range(len(self.team1))]
        ...
        for i in range(len(self.team1)):
            ...
            if self.cooldowns[i] <= 0:
                ...
                if not self.shot_wait[i]:
                    self.cooldowns[i] = random.randint(int(40 * (FRAME_RATE / 30)), int(80 * (FRAME_RATE / 30)))
    ...
    def update_charge(self, index):
        ...
        """
        if self.cooldowns[index] <= 9 * int(FRAME_RATE / 30):
            self.attack_forward[index] -= 1 * (30 / FRAME_RATE)
        elif self.cooldowns[index] <= 18 * int(FRAME_RATE / 30):
            self.attack_forward[index] += 1 * (30 / FRAME_RATE)
            if self.cooldowns[index] < 14 * int(FRAME_RATE / 30):
                self.attack_jump[index] -= 1 * (30 / FRAME_RATE)
            elif self.cooldowns[index] > 14 * int(FRAME_RATE / 30):
                self.attack_jump[index] += 1 * (30 / FRAME_RATE)
        else:
