...
class HeadToHeadTraining(Training):
    ...
    def __init__(self) -> None:
        ...
        # Sprite caching
        self._sprite_cache['head_training_img'] = sprite_load_percent(HEADTRAINING_PATH, percent=60, keep_proportion=True, base_on="width")
        self._sprite_cache['vs_img'] = sprite_load_percent(VS_PATH, percent=17, keep_proportion=True, base_on="width")
        self._sprite_cache['strikes'] = sprite_load_percent(STRIKE_PATH, percent=7, keep_proportion=True, base_on="width")
        self._sprite_cache['strikes_back'] = sprite_load_percent(STRIKES_BACK_PATH, percent=40, keep_proportion=True, base_on="width")
    ...
    def draw(self, surface: pygame.Surface):
        ...
        if self.phase != "result":
            ...
            starting_x = SCREEN_WIDTH - strikes_back.get_width() - int(5 * UI_SCALE)
            starting_y = SCREEN_HEIGHT - strikes_back.get_height() - int(5 * UI_SCALE)
    ...
    def draw_result(self, surface):
        ...
        blit_with_shadow(
            surface,
            head_training_img,
            (center_x - head_training_img.get_width() // 2, center_y - head_training_img.get_height() - int(20 * UI_SCALE))
        )
        ...
        total_width = wins_text.get_width() + vs_img.get_width() + losses_text.get_width() + int(20 * UI_SCALE)
        start_x = center_x - total_width // 2
        y = center_y + int(20 * UI_SCALE)

        blit_with_shadow(surface, wins_text, (start_x, y))
        blit_with_shadow(surface, vs_img, (start_x + wins_text.get_width() + int(10 * UI_SCALE), y))
        blit_with_shadow(surface, losses_text, (start_x + wins_text.get_width() + vs_img.get_width() + int(20 * UI_SCALE), y))
    ...
    def update_collision(self):
        ...
        target_x = SCREEN_WIDTH // 2 - int(12 * UI_SCALE)
        ...
        return (
            abs(left_x - target_x) <= int(10 * UI_SCALE)
            and abs(right_x - target_x) <= int(10 * UI_SCALE)
        )
